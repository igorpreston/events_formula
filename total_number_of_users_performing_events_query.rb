module EventsFormula
  class TotalNumberOfUsersPerformingEventsQuery < Base
    def count
      return 0 unless scope.any?

      external_user_ids(scope).size
    end
        ...
        @computed_data ||= begin
          {}.tap do |result|
            reduced_scope_by_day.each_with_index do |(date, _value), index|
              ...
              result[date] = reduced_values[index..end_point_index].flatten.uniq.size
            end
          end
        end
      end
    ...
    def scope_grouped_by_day
      @scope_grouped_by_day ||= begin
        all_counts = scope.group(:external_user_id)
